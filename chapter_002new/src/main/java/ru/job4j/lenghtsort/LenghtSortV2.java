...
/**
 * @task 3. Сортировка >3G файл [#860][#103649]
 * @author Konkin Anton
 * @version 3
 * @since 16.01.2019
 *
...
public class LenghtSortV2 implements ILenghtSort {
//    private final int maxPartSize = 100; // 100 kB
    private final int maxPartSize = 1024 * 1024; // 1MB
    ...
    public void sort(File source, File distance) {
        ...
        while (filesForSort.size() > 1) {
            File sorted = sortPairFiles(filesForSort.poll(), filesForSort.poll());
            filesForSort.addLast(sorted);
        }
